...
const sqlOutputPath = path.join(outputDirectory, "create_dim_replay.sql");
...
const replayJsonBase64 = Buffer.from(JSON.stringify(validRows), "utf8").toString("base64");
const replaySql = `-- Generated by analytics/scripts/build-replay-dimension.mjs.
-- Do not edit this output directly; update the canonical Replay Library source.

CREATE OR REPLACE TABLE \`terminus-maximus-analytics.terminus_analytics_dev.dim_replay\`
OPTIONS (
  description = 'Canonical valid Replay Library videos generated from repository-owned metadata.'
)
AS
WITH source AS (
  SELECT replay
  FROM UNNEST(JSON_QUERY_ARRAY(
    PARSE_JSON(CAST(FROM_BASE64('${replayJsonBase64}') AS STRING))
  )) AS replay
)
SELECT
  JSON_VALUE(replay, '$.video_id') AS video_id,
  JSON_VALUE(replay, '$.youtube_url') AS youtube_url,
  JSON_VALUE(replay, '$.creator') AS creator,
  JSON_VALUE(replay, '$.boss') AS boss,
  JSON_VALUE(replay, '$.tier') AS tier,
  SAFE_CAST(JSON_VALUE(replay, '$.guild_raid_season') AS INT64) AS guild_raid_season,
  JSON_VALUE(replay, '$.team_archetype') AS team_archetype,
  JSON_VALUE(replay, '$.hero_1') AS hero_1,
  JSON_VALUE(replay, '$.hero_2') AS hero_2,
  JSON_VALUE(replay, '$.hero_3') AS hero_3,
  JSON_VALUE(replay, '$.hero_4') AS hero_4,
  JSON_VALUE(replay, '$.hero_5') AS hero_5,
  JSON_VALUE(replay, '$.mow') AS mow,
  JSON_VALUE(replay, '$.map') AS map,
  SAFE_CAST(JSON_VALUE(replay, '$.damage') AS INT64) AS damage,
  SAFE_CAST(JSON_VALUE(replay, '$.published_date') AS DATE) AS published_date,
  JSON_VALUE(replay, '$.review_status') AS review_status,
  SAFE_CAST(JSON_VALUE(replay, '$.is_valid_replay') AS BOOL) AS is_valid_replay,
  SAFE_CAST(JSON_VALUE(replay, '$.source_exported_at') AS TIMESTAMP) AS source_exported_at,
...
await fs.writeFile(sqlOutputPath, replaySql, "utf8");
