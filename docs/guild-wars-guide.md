...
3. Update `edition`, infographic paths and true image dimensions in `editorial.json`; update tips there and methodology bullets in the page independently. Seasons, tiers, wars, and eligible attacks come from the new report audit. Check the edition, scope, title mapping, and infographic together before publishing. The card array supports future actual guides; don't add speculative cards.
...
## September 12, 2026 roster refresh

Accepted local report: `/Users/danfoster01/Documents/Websites/TacticusXYZ/analysis/attacker_core_discovery/2026-09-12-v4/index.html`.

Genestealers now requires Judh, Isaak, Hollan and Patermine (the request supplied four names despite saying five). Its canonical ID changed from `c_genesBiophagus_genesKelermorph_genesPrimus` to `c_genesBiophagus_genesKelermorph_genesPatriarch_genesPrimus`. Blood Angels now requires Mephiston, Dante, Mataneo, Nicodemus and Lucien; its ID changed from `c_bloodDante_bloodIntercessor_bloodMephiston_bloodSanguinary` to `c_bloodDante_bloodDeathCompany_bloodIntercessor_bloodMephiston_bloodSanguinary`. All ten section anchors and the other eight core mappings are preserved. The 33 named core heroes have no repeats; flex heroes may still occur in another core.

Main counts, main MoW usage, main exact tables and main map context require every core hero. Genestealers uses ordinary fifth-hero flex. Blood Angels additionally imports optional `flex_analysis` as `flexAnalysis`: the deduplicated union of eligible battles containing at least four of its five core heroes. This is 2,930 battles: 2,115 with the entire core and 815 with an external replacement. Each complete-core battle counts once. External flex usage divides by 2,930; named core heroes never become flex. Optional exact rows carry `omittedCore`, and the renderer only displays core heroes actually present in that row.

Blood Angels’ main popular-flex display reads this separate cohort. A separate disclosure presents its exact teams, team plus support, MoW usage, clear/Perfect/Meds Up metrics, and map context. The usual five-row limits and 3.8% flex/10% support cutoffs remain. The other eight normalized website team objects exactly match the pre-refresh dataset.

The independent research check reconstructed outcomes from raw canonical unit death states for all 77,733 scoped records, verified Medicae buff occurrences, MoW identities and map classifications, and retained 60,641 eligible battles including failures. War 6212 and the original V3 hashes reconcile. Website regression expectations now use Genestealers 2,886/3,033 clears and Blood Angels 1,839/2,115, with the exact metric and flex denominators checked against the complete report. The fixed-roster report does not claim a new optimizer run.
